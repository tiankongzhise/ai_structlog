...
| 核心依赖 | structlog>=23.1.0、python-jsonc-parser>=1.3.0、pydantic>=2.0 |
...
|--------|--------------|----------|----------|
| 读取默认配置文件 | 1. 默认路径：`项目根目录/structlog_config.json`<br>2. 使用 `python-jsonc-parser` 解析 JSONC（兼容注释）<br>3. 配置不存在时加载内置默认配置（封装在 `defaults.py`） | 1. 根目录无配置文件<br>2. 配置文件为空/仅注释<br>3. 配置文件权限为只读（但可读取） | 验证有/无配置文件时的加载行为，空配置降级 |
